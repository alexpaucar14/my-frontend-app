...
function loadScript(src) {
  return new Promise((resolve, reject) => {
    ...
    s.src = src;
    s.async = false; // mantiene el orden de ejecución
    s.onload = resolve;
    s.onerror = reject;
...
export default function Listar() {
  ...
  const initialized = useRef(false); // ⚡ Flag de inicialización
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ⚡ Evitar reinicialización en StrictMode
        ...
        // Guardar instancia para cleanup
        tableRef.current._dt = tableInstance;
      } catch (e) {
        console.error("Error cargando DataTables:", e);
      }
    })();

    return () => {
      try {
        if (tableRef.current?._dt) {
          tableRef.current._dt.destroy(true);
          tableRef.current._dt = null;
          initialized.current = false; // reset para cuando realmente desmontes
        }
      } catch { }
    };
  }, []);
